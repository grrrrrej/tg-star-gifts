...
2. **Загрузите проект:**
   ```bash
   git clone https://github.com/grrrrrej/tg-star-gifts.git
   cd tg-star-gifts
python -m venv venv
...
### 4. Быстрая Установка и Запуск (Linux / Android (Termux) и iOS (iSH))
...
### 5. Управление Скриптом, Менеджер `setup.sh` и Безопасность
