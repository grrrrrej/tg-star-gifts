...
async def send_main_menu():
    ...
    text = (f"✨ **Star Gifts Manager v5.7**\n\n"
            f"> 💎 Баланс: `{balance} ⭐` \n\n"
...
async def message_handler(event):
    ...
    if low_text == ".gift":
        ...
        msg = await event.respond("🎯 **Шаг 1: Кому?**\n\nВведи `@username` или `ID` получателя:", buttons=Button.clear())
    ...
    if state["step"] == "target":
        ...
        gifts = load_db()
        menu = "🎁 **Шаг 2: Выбери подарок**\n\n"
        btns = []
        for i, g in enumerate(gifts):
            menu += f"**{i+1}** ➔ `{g['name']}`\n"
            btns.append(Button.text(str(i+1), resize=True))
        menu += "\n> Или просто введи **ID** подарка"
...
async def start_bot():
    ...
    print("\033[94mБот v5.7 запущен! Иди в Избранное.\033[0m")
